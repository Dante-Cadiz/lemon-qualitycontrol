...
def resize_input_image(img: np.ndarray, version: str) -> np.ndarray:
    """
    Resize input image to specified dimensions based on image shape pickle
    file from chosen version
    """
...
def load_model_and_predict(
          my_image: np.ndarray, version: str) -> Tuple[float, str]:
    """
    ...
    target_map = {v: k for k, v in
                  {'Bad Quality': 0, 'Good Quality': 1}.items()}
